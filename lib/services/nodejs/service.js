...
var NodeService = { 
	...
	_check_args: function() {
		...
		if (argv.getconfig) {
			try { 
				this.load_config().then(function(config) { console.log(JSON.stringify(config));	});
			} catch(e) { 
			...
			if (argv.indxhost) {
				this.load_config().then(function(config) {
					console.debug('RUN load config --- ', argv.indxhost, config, typeof(config), config.length);
					console.debug('----------');
					nodeindx.login(argv.indxhost,config.user,config.password)
						.then(function(store) { this_.run(store); })
						.fail(function() { console.error('fail logging :('); });
				}).fail(function(err) { this_.debug('error loading config for run > ', err)});
			} else {
