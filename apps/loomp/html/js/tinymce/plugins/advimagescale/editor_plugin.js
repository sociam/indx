...
(function() {
    ...
    function constrainSize(ed, el, e) {
        ...
        if (lastDimensions && lastDimensions[elId] && lastDimensions[elId].width){ 
            resized = (dom.getAttrib(el, 'width') != lastDimensions[elId].width || dom.getAttrib(el, 'height') != lastDimensions[elId].height);
        } else {
            resized = false;
        }
        ...
        if (ed.getParam('advimagescale_append_to_url', true)) {
