...
class FitbitHarvester:
    ...
    def get_indx(self, server_url, box, user, password, appid):
        return_d = Deferred()
        def authed_cb(): 
            def token_cb(token):
    ...
    def harvest(self, server_url):
        ...
        self.get_indx(server_url, box, user, password, "INDX_Fitbit_Harvester").addCallbacks(indx_cb, lambda failure: logging.error("Error: {0}".format(failure)))
